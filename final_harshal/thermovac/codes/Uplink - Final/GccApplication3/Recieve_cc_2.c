...
//#include <uart.c>
//Global Variable
unsigned char data_transmit[60],data_receive[60] ,transmit_enable=0,receive_enable=1,SWITCH[8]="$SWITCH",switch_num=0,transmit_check=0;
unsigned char address[8]="$MANGO1";
...
// CC1101 
#define CC_GDO0	6	// Port B bit 0 (pin12) (pin6 of cc)
#define CC_GDO2 7   // Port B bit 1 (pin13) (pin3 of cc)
...
//void transmit_string_UART0(unsigned char *, unsigned char);
//unsigned char uart_getc(void);

void CC_Transmit();
 void  CC_Receive();

/*void transmit_UART0(unsigned char c) {
...
*****/
/*void transmit_string_UART0(char *s) {
	//  loop until *s != NULL
	while (*s) {
		transmit_UART0(*s);
		s++;
...
*****/
/*void transmit_string_UART0n(unsigned char *s, unsigned char n) {
	while (n>0) {
		transmit_UART0(*s);
		s++;
		n--;
...
//UART register config
	...
	UCSRA=0x00;
	UCSRB=0x98; //Enabling RX, TX and RX interrupt
*/
...
{
	data_transmit[check++]=UDR;
	if(check<8)
	{
		if(data_transmit[check-1]!=SWITCH[check-1])
			check=0;
	}
	else if (check==8)
		switch_num=data_transmit[check-1];
	else if(check==9){
	//if(check==9){
		transmit_enable=1;
		check=0;}
...
/************** Commented on 1/10/14 by Aayush*******************/
/************** due to malfunctoining of the code when burnt on atmega 8*******/
...
SIGNAL(SIG_USART_RECV) // ISR for receive complete interrupt
{
	...
	if(check<7)
	{
		if(data_transmit[check-1]!=address[check-1])
			check=0;
	}
	
	else if(check==8)
	{
		pkt_length=data_transmit[check-1]-48;
	}
	
	else if(check==pkt_length)
	{
		transmit_enable=1;
		check=0;
	}
...
void CC_Transmit(unsigned int pkt_length)
{
	...
	/*char y = ccxxx0_Read(CCxxx0_TXBYTES);
	transmit_UART0(y+48);
	
	strcpy((char*)temp,(const char*) "temp");
	transmit_string_UART0n((unsigned char*)temp, 8);
	transmit_string_UART0("reading TXFIFO");
	ccxxx0_ReadBurst(CCxxx0_TXFIFO, (unsigned char*)temp, 8 );
	transmit_string_UART0n((unsigned char*)temp, 8 );
...
void CC_Receive()
{
	...
	//transmit_UART0(transmit_enable);
	while(1)
	{
	...
	//		transmit_enable=0;
		...
		else if( PINB&(1 << CC_GDO0) )
		{
			...
			//transmit_string_UART0("RXed bytes: ");
			//transmit_UART0(y+48);
			//transmit_UART0(z);
			//transmit_UART0((i+48));
			//transmit_UART0((i));

function varargout = preflight(varargin)
% PREFLIGHT MATLAB code for preflight.fig
%      PREFLIGHT, by itself, creates a new PREFLIGHT or raises the existing
%      singleton*.
%
%      H = PREFLIGHT returns the handle to a new PREFLIGHT or the handle to
%      the existing singleton*.
%
%      PREFLIGHT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PREFLIGHT.M with the given input arguments.
%
%      PREFLIGHT('Property','Value',...) creates a new PREFLIGHT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before preflight_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      handles.stop.  All inputs are passed to preflight_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help preflight

% Last Modified by GUIDE v2.5 10-Jul-2015 00:43:41

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @preflight_OpeningFcn, ...
                   'gui_OutputFcn',  @preflight_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before preflight is made visible.
function preflight_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to preflight (see VARARGIN)

% Choose default command line output for preflight
...
% UIWAIT makes preflight wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = preflight_OutputFcn(hObject, eventdata, handles) 
...
for row1=1:row
    ...
    %if mod(row1,2)==0 %periodically write the data to a mat file (to avoid data loss)
        ...
        save(strcat(iString,'/HM_data.mat'),'HM');
        save(strcat(iString,'/OC_bits_data.mat'),'OC_bits');
        save(strcat(iString,'/Load_stat_bits_data.mat'),'Load_stat_bits');
        save(strcat(iString,'/SS_data.mat'),'SS');
        save(strcat(iString,'/GPS_data.mat'),'GPS');
        save(strcat(iString,'/Mag_data.mat'),'Mag');
        save(strcat(iString,'/Raw_data.mat'),'data');
    %end
    set(handles.gps_obs,'String',sprintf('%.0f', GPS1(1)));
    ...
    loadI = double(double(double(double(HM1(6)/255)*3.3)/0.005)/250);
